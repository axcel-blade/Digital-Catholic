import type { Saint } from '../types';

const saint: Saint = {
	"slug": "st-nicholas",
	"title": "St. Nicholas of Myra",
	"feastDays": "December 6",
	"excerpt": "Fourth-century bishop of Myra—patron of children, sailors, and the poor—whose secret charity and defense of Christ’s divinity inspired the beloved figure of Santa Claus.",
	"lifeDates": "c. 270–343",
	"sections": [
		{
			"heading": "Who is St. Nicholas?",
			"paragraphs": [
				"Saint Nicholas was a bishop of Myra in Lycia (modern-day Turkey) in the fourth century. One of the most popular saints in East and West, he is remembered for heroic charity, care for the poor, and steadfast faith in Jesus Christ as true God and true man.",
				"Very little is known with certainty about his life, yet ancient tradition, liturgy, and countless churches dedicated to him testify to his holiness. He died around the year 343 and was buried in Myra; his relics were later translated to Bari, Italy, where they are venerated to this day."
			]
		},
		{
			"heading": "His life and miracles",
			"paragraphs": [
				"Nicholas was born to a wealthy Christian family in Patara. After his parents died, he gave his inheritance to the needy. A famous story tells how he secretly provided dowries for three poor sisters so they would not be sold into destitution—slipping gold through a window or down a chimney, an episode that later shaped Christmas folklore.",
				"As bishop, Nicholas defended the faith. Eastern tradition holds that at the Council of Nicaea (325) he confronted the heretic Arius, who denied the divinity of Christ. He is also said to have rescued sailors in a storm, multiplied grain in famine, and raised three murdered boys to life—deeds that earned him patronage of children, travelers, and those in danger at sea.",
				"His tomb in Myra became a place of pilgrimage. When Italian merchants transferred his relics to Bari in 1087, devotion spread across Europe. Churches, schools, and cities from Russia to the Americas bear his name."
			]
		},
		{
			"heading": "Full biography",
			"paragraphs": [
				"Nicholas was born around 270 in Patara, Lycia (in modern Turkey), to wealthy Christian parents who died in an epidemic while he was young. He took Jesus’ words to heart, sold his inheritance, and gave the proceeds to the needy—beginning a life marked by secret generosity.",
				"While still young he was chosen bishop of Myra. During the persecution under Diocletian he was imprisoned and tortured for the faith, yet continued to defend Christians. After Constantine legalized Christianity, Nicholas attended the Council of Nicaea in 325 and, according to Eastern tradition, confronted the heretic Arius over the divinity of Christ.",
				"As bishop he rescued the falsely accused, multiplied grain in famine, and calmed storms at sea—deeds that shaped his patronage of sailors, prisoners, and children. His tomb in Myra became a place of pilgrimage until Italian merchants translated his relics to Bari in 1087."
			]
		},
		{
			"heading": "Death and legacy",
			"paragraphs": [
				"Nicholas died on December 6, 343, in Myra and was buried in his cathedral. The anniversary of his death became his feast day, still celebrated across East and West. Myrrh is said to flow from his relics in Bari, a sign of holiness venerated for nearly seventeen centuries.",
				"His charity inspired the figure of Santa Claus, but the saint himself points to Christ. Churches, cities, and nations claim him as patron; the Basilica of St. Nicholas in Bari remains one of the most visited shrines in Italy."
			]
		},
		{
			"heading": "From saint to Santa Claus",
			"paragraphs": [
				"The figure of “Santa Claus” grew from devotion to St. Nicholas, not the other way around. In the Netherlands, Sinterklaas (Saint Nicholas) still arrives on December 6, the eve of his feast, as a bishop in red vestments who brings gifts to children and remembers the poor.",
				"Dutch settlers brought the custom to America, where “Sinter Klaas” became “Santa Claus.” Nineteenth-century poems and illustrations—especially Clement Clarke Moore’s “A Visit from St. Nicholas” and the work of Thomas Nast—blended the saint with winter folk imagery. Commercial culture later added the sleigh, reindeer, and North Pole workshop.",
				"Catholics need not reject family Christmas customs, but we can recover their root: a holy bishop who gave secretly because he loved Christ in the poor. Teaching children about the real St. Nicholas keeps generosity tied to the Gospel rather than to mere consumption."
			]
		},
		{
			"heading": "Patronage and devotion",
			"paragraphs": [
				"St. Nicholas is patron of children, sailors, merchants, archers, repentant thieves, brewers, pawnbrokers, and many cities and nations. His feast on December 6 opens the Advent season in much of the Church, reminding the faithful that Christmas celebrates the birth of Christ, not only gift-giving.",
				"In the East he is honored among the greatest saints; in the West his cult flourished in the Middle Ages. Many families bless children on his feast, leave shoes for small gifts, or give alms in his name. The Basilica of St. Nicholas in Bari remains a major shrine."
			]
		},
		{
			"heading": "Lessons for us today",
			"paragraphs": [
				"Nicholas teaches us to give without seeking praise, to defend the truth of Christ with courage and charity, and to notice Christ in those who are poor or forgotten. His life shows that holiness can be joyful, practical, and hidden.",
				"Whether you call him St. Nicholas, Sinterklaas, or Santa, he points beyond himself to the Child of Bethlehem. Ask his intercession for a generous heart—and for the grace to keep Christmas centered on Christ."
			]
		}
	]
};

export default saint;
